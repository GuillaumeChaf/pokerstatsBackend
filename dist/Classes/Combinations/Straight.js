...
class Straight {
    static checkCombination(combinationCard) {
        ...
        if (combinationCard[0][0].value == "A")
            combinationCard.push([{ value: "aa", symbol: "" }]); //add ace to check straight 5-4-3-2-A
        let serie = 1;
        if (combinationCard.length < 5)
            return false;
        for (let i = 1; i < combinationCard.length; i++) {
            if ((cardValued.indexOf(combinationCard[i - 1][0].value)) - (cardValued.indexOf(combinationCard[i][0].value)) === -1) {
                serie++;
            }
            else {
                serie = 1;
    ...
    static calcul(combinationCard) {
        ...
        if (combinationCard[0][0].value == "A")
            combinationCard.push([{ value: "aa", symbol: "" }]); //add ace to calcul straight 5-4-3-2-A
        ...
        for (let i = 1; i < combinationCard.length; i++) {
            ...
            ;
            if (serie === 1)
                leadStraight = cardValuedReverse.indexOf(combinationCard[i][0].value);
            if (serie > 4)
                break;
        }
