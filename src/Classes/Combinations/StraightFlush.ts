...
export default class StraightFlush{
  ...
  static checkCombination(combinationCard : Card[][]) : boolean{
    ...
    if(combinationCard[0][0].value == "A") combinationCard[0].push({value : "aa", symbol : ""});//add ace to check straight 5-4-3-2-A
    let serie = 1;

    if(combinationCard[0].length < 5) return false;
    for(let i = 1; i < combinationCard[0].length; i++){
      if((cardValued.indexOf(combinationCard[0][i-1].value)) - (cardValued.indexOf(combinationCard[0][i].value)) === -1){ serie++ } else { serie = 1};
  ...
  static calcul(combinationCard : Card[][]) : number {
    ...
    if(combinationCard[0][0].value == "A") combinationCard[0].push({value : "aa", symbol : ""});//add ace to calcul straight 5-4-3-2-A
    ...
    for(let i = 1; i < combinationCard.length; i++){
      if((cardValued.indexOf(combinationCard[0][i-1].value)) - (cardValued.indexOf(combinationCard[0][i].value)) === -1){ serie++ } else { serie = 1};
      if(serie === 1) leadStraight = cardValuedReverse.indexOf(combinationCard[0][i].value);
      if(serie > 4) break;
    }
